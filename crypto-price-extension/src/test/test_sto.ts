...
async function testOkx(symbol: string) {
...
async function testBinance(symbol: string) {
...
async function testHyperliquid(symbol: string) {
    ...
    try {
        ...
        if (idx >= 0) {
            ...
            const sCoin = universe.filter((u: any) => u.name.startsWith('S')).map((u: any) => u.name);
