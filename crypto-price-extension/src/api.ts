...
import { fetchHyperliquidPrices } from './hyperliquid';
...
export interface PriceData {
    ...
    lowUtc0: string;
    highUtc0: string;
    amplitudeUtc0: string;
    fundingRate?: string;
}

const INDEX_SYMBOLS = ['SPX'] as const;
 ...
 */
function toBasePair(symbol: string): string {
 ...
 */
async function fetchDailyCandleUtc0(instId: string): Promise<{ high: string; low: string } | null> {
 ...
 */
async function tryFetchTicker(url: string): Promise<any | null> {
    try {
 ...
 */
async function resolveTicker(symbol: string): Promise<{ ticker: any; candleInstId: string; isIndex: boolean } | null> {
    if (isIndexSymbol(symbol)) {
        const ticker = await tryFetchTicker(
            `https://www.okx.com/api/v5/market/index-tickers?instId=${toBasePair(symbol)}`,
        );
    ...
    // Try spot first
    const spotTicker = await tryFetchTicker(
        `https://www.okx.com/api/v5/market/ticker?instId=${basePair}`,
    );
    if (spotTicker) {
    ...
    const swapTicker = await tryFetchTicker(
...
async function fetchSingleSymbol(
    ...
    try {
        ...
        if (!isIndex && candleInstId) {
            ...
            try {
                const frRes: any = await (await fetch(`https://www.okx.com/api/v5/public/funding-rate?instId=${basePair}-SWAP`)).json();
...
export async function fetchPrices(symbols: string[]): Promise<Map<string, PriceData>> {
    ...
    if (cryptoSymbols.length > 0) {
        ...
        if (missingSymbols.length > 0) {
            ...
            await fetchBinancePrices(missingSymbols, results);
        }
        ...
        if (missingSymbols.length > 0) {
            ...
            await fetchHyperliquidPrices(missingSymbols, results);
        }
    }
